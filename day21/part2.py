...
@cache
def get_presses(sequence, depth=25, is_directional=False, current=None):
    """
...
def calculate_part2(codes):
    """Calculate the sum of complexities for all codes with 25 robots."""
    return sum(int(code[:-1]) * get_presses(code, depth=25) for code in codes)

if __name__ == "__main__":
    input_file = sys.argv[1] if len(sys.argv) > 1 else 'input.txt'
    codes = read_input(input_file)
    result = calculate_part2(codes)
    print(result)
